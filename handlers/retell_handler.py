...
async def retell_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> None:
    ...
    try:
        ...
        # Генерируем пересказ
        logger.info(f"Отправляем текст пользователя {user_id} в OpenRouter API для пересказывания")
        retelling = generate_text_retelling(text)

        if retelling:
            response = f"📝 Современный пересказ:\n\n{retelling}"
            logger.info(f"OpenRouter API успешно вернул пересказ для пользователя {user_id} (длина: {len(retelling)} символов)")
            ...
            logger.error(f"OpenRouter API не смог пересказать текст для пользователя {user_id}")
            response = (
